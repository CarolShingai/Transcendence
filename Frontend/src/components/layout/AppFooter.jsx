    ...
    <footer className="App-footer" style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', alignItems: 'center', justifyContent: 'center' }}>
        <button
          type="button"
          onClick={() => {
            try { if (typeof onGoToPrivacyPolicy === 'function') onGoToPrivacyPolicy(); } catch (e) {}
            try { window.dispatchEvent(new CustomEvent('open:privacy-overlay')); } catch (e) {}
          }}
        ...
        <button
          type="button"
          onClick={() => {
            try { if (typeof onGoToTermsOfUse === 'function') onGoToTermsOfUse(); } catch (e) {}
            try { window.dispatchEvent(new CustomEvent('open:terms-overlay')); } catch (e) {}
          }}
          style={{ background: 'none', border: '1px solid #fff', color: '#fff', padding: '4px 8px', cursor: 'pointer' }}
        >
          Termos de Uso
